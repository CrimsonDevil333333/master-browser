...
export default function MasterBrowser() {
  ...
  const renderFileList = (filesToRenderRaw: FileMetadata[], pathSetter: (p: string) => void) => {
    const filtered = filesToRenderRaw.filter(f => f.name.toLowerCase().includes(searchQuery.toLowerCase()));
    return (
      <div className="flex-1 w-full min-h-[500px] relative">
        {/* @ts-ignore */}
        <AutoSizer>
          {({ height, width }: { height: number; width: number }) => (
            <VirtualList height={height} width={width} itemCount={filtered.length} itemSize={75}>
              {({ index, style }: { index: number; style: any }) => {
                const f = filtered[index];
                return (
                  <div style={style} className="px-1 py-1">
                    <div onClick={() => f.is_dir ? setCurrentPath(f.path) : openFile(f)} className="flex items-center gap-4 p-4 rounded-2xl bg-zinc-50 dark:bg-zinc-900/40 border border-zinc-200 dark:border-zinc-800 hover:border-indigo-500/50 cursor-pointer transition-all">
                        <div className={cn("p-2 rounded-lg", f.is_dir ? "text-indigo-500 bg-indigo-500/10" : "text-zinc-400 bg-zinc-100 dark:bg-zinc-800")}>
                            {f.is_dir ? <Folder className="w-4 h-4" /> : <FileIcon className="w-4 h-4" />}
                        </div>
                        <div className="flex-1 min-w-0">
                            <p className="text-sm font-bold truncate text-zinc-900 dark:text-zinc-100">{f.name}</p>
                            <p className="text-[9px] text-zinc-500 font-mono uppercase">{f.is_dir ? 'Folder' : formatSize(f.size)}</p>
                        </div>
  ...
  return (
    <div className={cn("flex h-screen w-screen transition-colors duration-300", theme === 'dark' ? "bg-[#050505] text-white" : "bg-white text-zinc-900")}>
      ...
      <aside className="w-72 border-r border-zinc-200 dark:border-zinc-800 flex flex-col p-8 shrink-0 bg-zinc-50/50 dark:bg-[#080808] z-50">
        <div className="flex items-center gap-4 mb-12">
            <div className="p-3 bg-indigo-600 rounded-2xl shadow-lg"><Shield className="w-6 h-6 text-white" /></div>
            <div><h1 className="text-xl font-black tracking-tighter text-zinc-900 dark:text-zinc-100">MASTER</h1><p className="text-[8px] font-bold text-indigo-500 uppercase">Sovereign Alpha</p></div>
        </div>
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-white dark:bg-[#050505]">
        <header className="h-24 px-12 border-b border-zinc-100 dark:border-zinc-900 flex items-center justify-between z-40 bg-white dark:bg-[#050505]">
            ...
            <div className="flex items-center gap-6 text-zinc-900 dark:text-zinc-100">
                <div className="relative"><Search className="w-4 h-4 absolute left-4 top-1/2 -translate-y-1/2 text-zinc-400" /><input type="text" placeholder="Search..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl pl-11 pr-4 py-2.5 text-xs font-bold outline-none w-64" /></div>
        ...
        <div className="flex-1 p-12 overflow-hidden flex flex-col relative z-30">
                ...
                <motion.div key={view} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="flex-1 flex flex-col">
                    {view === 'explorer' && (
                        <div className="flex-1 flex flex-col gap-6">
                            <div className="flex items-center justify-between bg-zinc-100 dark:bg-zinc-900/50 p-4 rounded-2xl border border-zinc-200 dark:border-zinc-800 text-zinc-900 dark:text-zinc-100">
                                <div className="flex items-center gap-4">
                                    <button onClick={() => navigateUp()} className="p-2 hover:bg-zinc-200 dark:hover:bg-zinc-800 rounded-lg transition-all"><ArrowLeft className="w-4 h-4" /></button>
                                    <p className="text-xs font-mono font-bold truncate max-w-lg">{currentPath}</p>
                                </div>
                    ...
                    {view === 'terminal' && (
                        <div className="flex-1 bg-[#080808] rounded-[2.5rem] border border-zinc-800 flex flex-col overflow-hidden shadow-2xl">
                            ...
                            <div className="flex-1 p-8 font-mono text-[11px] overflow-auto custom-scrollbar text-zinc-400 space-y-1">{terminalOutput.map((l, i) => <p key={i} className={l.startsWith('>') ? "text-indigo-400" : ""}>{l}</p>)}</div>
                            <div className="p-5 bg-zinc-950/60 border-t border-white/5 flex items-center gap-4"><span className="text-indigo-500 font-bold ml-2">λ</span><input autoFocus className="flex-1 bg-transparent border-none outline-none text-[11px] font-mono text-zinc-100" value={terminalInput} onChange={e => setTerminalInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && runCommand()} placeholder="..." /></div>
                        </div>
                    )}
                    {view === 'raw' && <RawDiskViewer />}
                    {view === 'settings' && (
                        <div className="p-10 bg-zinc-100 dark:bg-zinc-900/40 border border-zinc-200 dark:border-zinc-800 rounded-3xl space-y-6 text-zinc-900 dark:text-zinc-100"><h3 className="text-xl font-black">Settings</h3><div className="flex items-center justify-between p-6 bg-white dark:bg-black/20 rounded-2xl border border-zinc-200 dark:border-white/5"><span className="font-bold text-zinc-600 dark:text-zinc-400">Interface Theme</span><button onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')} className="px-6 py-2 bg-indigo-600 text-white rounded-lg font-bold text-xs uppercase tracking-widest">{theme} Mode</button></div></div>
                    )}
                </motion.div>
            </AnimatePresence>
        </div>
